...
def course_dfs(current):
    ...
    for li in page.find_all('li'):
        if li.get('id', None):
            course_dfs(li)
        elif not "earlier" in current.text.strip():
            a_tag = li.find('a')
            findings.append({
                "degree_id": int(re.search(r'\d+', a_tag.get('href')).group()),
                "name": a_tag.text.strip(),
                "parent": current.text.strip()
            }) 
...
class UniversityOfSydney(AbstractScraper):
    ...
    def units(self):
        units = []
        page = scrape_page("https://www.sydney.edu.au/s/search.html?query=!Z&collection=Sydney-Curriculum_UOS&start_rank=1&form=simple&num_ranks=9999999")
        ...
        for u in raw_units:
            ...
            for i in range(len(dt)):
                ...
                elif "description" in dt[i].text:
                    unit['description'] = dd[i].text.strip()
                elif "title" in dt[i].text:
                    unit['name'] = dd[i].text.strip()
            units.append(unit)
        return sorted(units, key=lambda unit: unit['code'])

    def course_detail(self, *args, **kwargs):
        degree_id = kwargs.get('degree_id', None)
        page = scrape_page("https://cusp.sydney.edu.au/students/view-degree-page/degree_id/%d" % degree_id)
        course_detail = {}
    ...
    def unit_detail(self, *args, **kwargs):
        ...
        try:
            link = "https://cusp.sydney.edu.au/students/view-unit-page/alpha/{}".format(code)
            unit_page = scrape_page(link)

            unit_detail['link'] = link

            if unit_page.title.text == "Unit - The University of Sydney":
            ...
            elif "CUSP" in unit_page.title.text:
                overview = unit_page.find('div', id='overview')

                if overview is None:
                    raise HTTPError

                overview = overview.find_all('tr')                 
